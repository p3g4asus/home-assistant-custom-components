...
LEARN_COMMAND_SCHEMA = vol.Schema({
    ...
    vol.Optional(CONF_TIMEOUT, default=DEFAULT_LEARN_TIMEOUT): vol.All(int, vol.Range(min=0)),
...
async def async_setup_platform(hass, config, async_add_entities,
    ...
    for _,data in remote_data.items():
        if "obj" not in data:
            data["obj"] = AllOne((data.get(CONF_HOST),PORT), mac=data.get(CONF_MAC))
...
class AllOneRemote(RemoteDevice):
    ...
    async def _send_command(self, packet, totretry):
        ...
        for _ in range(num):
            ...
            await self._device.emit_ir(payload)
        return False
